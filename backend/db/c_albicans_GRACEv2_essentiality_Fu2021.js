...
async function grace2DataImageRelated(FilePath, DBFilePath) {
  ...
  db.serialize(() => {
    const dropTableSql = `DROP TABLE IF EXISTS GraceV2ImageRelatedTable`;
    db.run(dropTableSql, (err) => {
      if (err) {
        console.error(err.message);
        return;
      }
      const createTableSql = `CREATE TABLE GraceV2ImageRelatedTable (${columns.join(
        ", "
      )})`;
      db.run(createTableSql, (err) => {
        ...
        data.slice(1).forEach((row, i) => {
          try {
            ...
            const sql = `INSERT INTO GraceV2ImageRelatedTable (${columnNames.join(
              ", "
            )}) VALUES (${placeholders})`;
