...
async function insert_SI_Kinase_DataFromExcel(excelFilePath, dbFilePath) {
  return new Promise((resolve, reject) => {
    ...
    db.serialize(() => {
      const dropTableSql = `DROP TABLE IF EXISTS SI_Kinase_Table`;
      db.run(dropTableSql, (err) => {
        ...
        const createTableSql = `CREATE TABLE SI_Kinase_Table (${columns.join(
          ", "
        )})`;
          ...
          const insertPromises = jsonData.slice(1).map((row, i) => {
            return new Promise((insertResolve) => {
              ...
              const placeholders = sanitizedColumnNames
                .map(() => "?")
                .join(",");
              const sql = `INSERT INTO SI_Kinase_Table (${sanitizedColumnNames.join(
                ", "
              )}) VALUES (${placeholders})`;
...
module.exports = {
    insert_SI_Kinase_DataFromExcel,
};
...
// (async () => {
//   try {
//     await insert_SI_Kinase_DataFromExcel(excelFilePath, dbFilePath);
