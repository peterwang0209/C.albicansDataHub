  ...
  <div>
    <div class="flex justify-center">
      <form @submit.prevent="search" class="relative flex items-center">
        <select
          class="border border-gray-300 bg-white text-gray-600 mr-2 w-40 h-10 pl-5 pr-10 rounded-lg focus:outline-none"
          v-model="searchType"
        >
        ...
        <input
          type="search"
          class="block w-full p-4 pl-10 h-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
          placeholder="Search..."
          v-model="searchTerm"
          required
        />
...
export default {
  ...
  methods: {
    search(searchItem = { term: this.searchTerm, type: this.searchType }) {
      ...
      this.$store.commit("addSearch", searchItem);
      this.$router.push({
        name: "DataPage",
