...
export default {
  ...
  methods: {
    createHistogram() {
      ...
      const dataAttribute = this.attributeName;

      const values = this.graphData.map((d) => {
        const value = d[this.attributeName];
        const number = parseFloat(value); // Convert the value to a floating-point number
        return isNaN(number) ? 0 : number; // Replace non-numeric values with 0 (or any default value you prefer)
      });
